...
In questa sezione per le tre tipologie di dataset (completo, ridotto e trasformato), descriveremo alcune considerazione dei cluster costruiti con i diversi metodi, cercando di evidenziarne le differenze nella caratteristiche apprese. Successivamente analizzeremo la qualità dei diversi metodi di clustering, fissando il numero di cluster pari al numero di classi reali, ovvero 2. Questo approccio ci permette di utilizzare tecniche di validazione esterne, dato che disponiamo delle etichette reali delle osservazioni (variabile 'Status').  
...
### 6.1.1. Validazione con la classe reale
...
### 6.1.2. Interpretazione dei risultati

```{r}
# Assicuriamoci di avere le librerie necessarie
library(ggplot2)
library(reshape2)

# Seleziona le variabili numeriche da visualizzare (ad es. "Bottom", "Diagonal", e altre)
# Qui usiamo solo "Bottom" e "Diagonal" come esempio; se usi il dataset completo, includi anche le altre variabili.
vars_to_plot <- c("Bottom", "Diagonal")

# Crea un data frame in formato lungo per i boxplot, per il clustering K-means
boxplot_data <- melt(banknote, 
                     id.vars = c("Status", "kmeans"), 
                     measure.vars = vars_to_plot)

# Crea il boxplot
ggplot(boxplot_data, aes(x = factor(kmeans), y = value, fill = factor(kmeans))) +
  geom_boxplot() +
  facet_wrap(~variable, scales = "free") +
  labs(title = "Boxplot delle variabili per cluster (K-means)",
       x = "Cluster (K-means)",
       y = "Valore") +
  theme_minimal()
...
### 6.2.1. Validazione con la classe reale
...
### 6.3.1. Validazione con la classe reale
