...
packages <- c("mclust", "reshape2", "ggplot2", "GGally", "dplyr", "clustertend", "cluster", "FNN", "FactoMineR", "factoextra", "caret", "pheatmap", "clusterCrit", "NbClust", "fpc", "scatterplot3d", "gridExtra")
...
```
La matrice di scatter plot mostra una parziale separazione tra i due gruppi (`Status`), visibile sia nelle distribuzioni univariate che nelle relazioni bivariate.
Sebbene vi sia sovrapposizione in diverse aree, alcune coppie di variabili, in particolare `Bottom` e `Diagonal`, evidenziano una migliore distinzione tra i gruppi.
Questo suggerisce la presenza di una struttura nei dati che potrebbe essere rilevata più chiaramente considerando più variabili insieme
...
Assume valori compresi tra 0 e 1, più il valore è vicino allo 0 più il dataset è strutturato in cluster.
...
Nel grafico del dataset banknote, notiamo che lungo la diagonale sono presenti regioni rosse che indicano dati simili tra loro, suggerendo la presenza di cluster. Al contrario, il dataset randomico appare privo di struttura.
...
Nel grafico il gomito sembra collocarsi sulla seconda componente.

Andando per maggioranza, sulla base dei dati ottenuti, si sceglie di mantenere un numero di componenti principali pari a 2.
Di seguito viene costruito il nuovo dataset in cui le variabili sono sostituite con le prime 2 componenti principali, che verrà usato più avanti nelle analisi.
...
Osserviamo che le variaibili che appaiono con colori più accesi sono `Length` che contribuisce molto a Dim.2, e le variabili `Diagonal`, `Left` e `Right` con un contributo importante a Dim.1 seppure opposto.
...
Si osserva che `Length` e `Top` hanno contributi particolarmente elevati rispettivamente a Dim.2 e Dim.3.
...
Notiamo un ampio salto a circa 30 suggerisce una buona separazione in 2 cluster, mentre un altro salto meno marcato (intorno a 15) ne suggerisce 3.
Per confermare il numero ottimale di cluster, si prosegue con metodi alternativi.
...
```
Tra k = 1 e k = 2, la diminuzione del WSS è molto forte, tra k = 2 e k = 3 c'è ancora un calo del WSS ma meno netto.
Oltre k = 3, la pendenza inizia a ridursi gradualmente. Dal grafico, sembra che il calo principale avvenga per k = 2, mentre oltre si riduce più lentamente.
...
Da k = 3 l'indice diminuisce senza tornare ai valori di picco.
...
Complessivamente la metà dei metodi analizzati (gap statistic, nbclust) converge su una soluzione a 3 cluster, mentre l'altra metà (elbow, e silhouette) converge a 2 cluster.
Pertanto non possiamo optare per una scelta di "maggioranza", decidiamo di scegliere un numero di cluster pari a 2 poichè il salto più marcato nel dendrogramma indica una forte separazione naturale in 2 gruppi.
...
clusters <- cutree(hclust_clustering, k = 2)
...
sil <- silhouette(clusters, distanze)
...
dunn_index <- intCriteria(as.matrix(df), clusters, "Dunn")$dunn
...
clust_stats <- cluster.stats(d = dist(df), status, clusters)
print(paste("Corrected Rand Index:", clust_stats$corrected.rand))
print(paste("Meila's Index:", clust_stats$vi))
```

Un Corrected Rand Index di 0.9602 è molto elevato, il che indica che il clustering ottenuto corrisponde molto bene alla partizione di riferimento.
Inoltre, un Information Index di 0.098 è basso, suggerendo che la differenza informativa tra le partizioni è minima.
...
Ottimizziamo il clustering esplorando diverse combinazioni di metodi di linkage e numero di cluster. Valutiamo ciascuna combinazione mediante le metriche viste prima.
Successivamente, assegniamo un ranking per ciascuna metrica (invertendo il criterio per quelle da massimizzare) e sommiamo i punteggi.
Infine, ordiniamo i risultati e selezioniamo la combinazione con il punteggio complessivo più basso come miglior clustering.
...
print(best_overall)
```
Il risultato mostra che il clustering ottimale secondo queste metriche è Ward.D2 con 2 cluster, che è la combinazione che abbiamo testato precedentemente.
...
Applicando il clustering agglomerativo con metodo "centroid" su 2 componenti principali (PCA) e 4 cluster, si ottiene la combinazione migliore, con Corrected Rand Index e VI ai primi posti, ma le metriche interne indicano una struttura non molto netta (basso Dunn e silhouette moderata).
Rispetto al clustering su tutte le variabili, la soluzione con PCA ha una silhouette leggermente migliore, ma risulta inferiore negli altri indici (Dunn, Corrected Rand, VI), suggerendo che la riduzione dimensionale migliora la coesione interna, ma non necessariamente la separazione o l'aderenza alla struttura reale.

Vediamo graficamente come vengono separati i due cluster con la configurazione trovata.
```{r}
final_pca <- eclust(banknote_pca_num, "hclust", k = 4, hc_metric = "euclidean", hc_method = "centroid", graph = FALSE)

fviz_cluster(final_pca, geom = "point", ellipse.type = "norm", palette="jco", ggtheme = theme_minimal())
...
```
Utilizzando solo le variabili Bottom e Top, la combinazione migliore (linkage ward.D2, k = 2) ottiene il valore silhouette più alto (0.6245), indicando cluster ben distinti.
Sebbene l'indice Dunn sia inferiore rispetto agli altri approcci, il Corrected Rand resta molto elevato (0.9602) e il VI basso, confermando una buona coerenza con la struttura attesa. Questo supporta l'idea che le due variabili selezionate siano efficaci nel separare i gruppi.
...
In questo capitolo analizzeremo,  il clustering parziale che è una tecnica di apprendimento non supervisionato che mira a suddividere un dataset in gruppi distinti, detti cluster, senza sovrapposizioni. A differenza del clustering gerarchico, che costruisce una struttura ad albero, il clustering parziale assegna ogni punto dati a un unico cluster, ottimizzando un criterio di similarità.

Gli algoritmi che utilizzeremo sono il k-means che è uno dei più utilizzati per la sua semplicità ed efficienza e il k-medoids che supera alcune limitazioni del k-means.

## 4.1. K-Means

In questo capitolo, applicheremo l'algoritmo k-means sui tre dataset derivati dal dataset *banknote*, il dataset completo, quello contenente solo le variabili `Bottom` e `Diagonal`, infine quello trasformato tramite PCA e ne analizzeremo i risultati.
...
Ed infine il GAP statistic method:
```{r}
set.seed(123)
...
Valutiamo la qualità del clustering prodotto dall'algoritmo k-means calcolando il **silhouette score**, il **Dunn index**, il **Corrected Rand Index** e il **Meila’s VI index**.

```{r}
fviz_silhouette(km.res, palette = "jco",
                ggtheme = theme_classic())
```
Un valore silhouette medio di 0.38 indica una qualità di clustering moderata, ma non particolarmente forte.  
Calcoliamo adesso il dunn index.

```{r}
km_stats <- cluster.stats(distanze, km.res$cluster)
km_stats$dunn                           
```
Un Dunn index pari a 0.1124569 suggerisce che la qualità del clustering non è particolarmente elevata.  
Passiamo a calcolare il corrected rand index.
```{r}
clust_stats <- cluster.stats(distanze, status, km.res$cluster)
clust_stats$corrected.rand
```

Un Corrected Rand Index di 0.8456292 indica un'elevata concordanza tra la partizione ottenuta dal clustering e la classificazione di riferimento.  
Infine, calcoliamo il Meila's VI index.
```{r}
clust_stats$vi
```

Un Meila’s VI index di 0.2819728 indica una discreta somiglianza tra la partizione ottenuta dal clustering e quella di riferimento.
...
I risultati ottenuti con i primi due metodi, elbow e silhouette, coincidono con quelli ottenuti sul dataset completo, mentre il gap statistic method identifica 2 cluster.

```{r}
km.res <- eclust(df_ridotto, "kmeans", k = 2, nstart = 25, graph = FALSE)
fviz_cluster(km.res, geom = "point", ellipse.type = "norm",
             palette = "jco", ggtheme = theme_minimal())
```
...
Procediamo, anche nel caso del dataset ridotto, a valutare la qualità del clustering k-means calcolando gli indici il **silhouette score**, il **Dunn index**, il **Corrected Rand Index** e il **Meila’s VI index**.
...
Un average silhouette width di 0.62 indica una buona qualità del clustering. Questo valore suggerisce che, in media, i punti sono ben assegnati ai loro cluster, con una chiara separazione rispetto agli altri gruppi.
Procediamo a calcolare il dunn index.  
```{r}
km_stats <- cluster.stats(distanze, km.res$cluster)
km_stats$dunn                           
```

Un Dunn index pari a 0.1427453 suggerisce che la qualità del clustering non è particolarmente elevata.
Continuiamo con il calcolo dell Corrected Rand Index.
```{r}
clust_stats <- cluster.stats(distanze, status, km.res$cluster)
clust_stats$corrected.rand
```

Essendo molto vicino a 1, suggerisce che quasi tutte le coppie di osservazioni sono state correttamente assegnate allo stesso cluster o a cluster diversi in entrambe le partizioni.
Procediamo a calcolare l'ultimo indice.

```{r}
clust_stats$vi
```

Anche qui il valore indica una somiglianza molto elevata tra la partizione ottenuta dal clustering e quella di riferimento.
...
Anche in questo caso il numero di cluster ottimale è pari a 2.

```{r}
final_pca <- eclust(df_pca, "kmeans", k = 2, nstart = 25, graph = FALSE)

fviz_cluster(final_pca, geom = "point", ellipse.type = "convex", palette="jco", ggtheme = theme_minimal())
```

#### 4.1.3.2. Validazione

Procediamo, anche nel caso del dataset con le variabili ricavate tramite mediante la PCA, a valutare la qualità del clustering k-means calcolando gli indici il **silhouette score**, il **Dunn index**, il **Corrected Rand Index** e il **Meila’s VI index**.

```{r}
fviz_silhouette(final_pca, palette = "jco",
                ggtheme = theme_classic())
```
Un average silhouette index di 0.53 indica una qualità di clustering discreta,il clustering mostra una separazione relativamente buona tra i cluster.

```{r}
km_stats <- cluster.stats(distanze, final_pca$cluster)
km_stats$dunn                           
```
...
```{r}
clust_stats <- cluster.stats(distanze, status, final_pca$cluster)
clust_stats$corrected.rand
```

Questo risultato suggerisce che quasi tutte le coppie di osservazioni sono state correttamente assegnate allo stesso cluster.

```{r}
clust_stats$vi
```

Il valore non è particolarmente basso, suggerendo che i cluster individuati non riproducono fedelmente la struttura sottostante ai dati originali. 
...
```{r}
pam.res <- eclust(df, "pam", k = 2, nstart = 25, graph = FALSE)
...
Valutiamo la qualità del clustering prodotto dall'algoritmo k-medoids calcolando il **silhouette score**, il **Dunn index**, il **Corrected Rand Index** e il **Meila’s VI index**.  
```{r}
...
```{r}
pam_stats <- cluster.stats(distanze, pam.res$cluster)
pam_stats$dunn                           
```

Un Dunn index pari a 0.1299438 suggerisce che la qualità del clustering non è particolarmente elevata.  
Passiamo a calcolare il corrected rand index.

```{r}
clust_stats <- cluster.stats(distanze, status, pam.res$cluster)
clust_stats$corrected.rand
```

Un Corrected Rand Index di 0.9406018 indica un livello di concordanza molto alto tra la partizione generata dal clustering e quella di riferimento.
Procediamo a a calcolare il Meila's VI index.

```{r}
clust_stats$vi
```

Anche in questo caso, il risultato indica una buona somiglianza tra la partizione ottenuta dal clustering e quella di riferimento.
...
Anche in questo caso il numero ottimale di cluster è 2.

```{r}
pam.res <- eclust(df_ridotto, "pam", k = 2, nstart = 25, graph = FALSE)
fviz_cluster(pam.res, geom = "point", ellipse.type = "norm",
             palette = "jco", ggtheme = theme_minimal())
```
...
Si procede alla validazione del dataset ridotto calcolando gli indici come fatto nei capitoli precedenti.
...
Il risultato è identico al quello calcolato per il k-means stando ad indicare che l'algoritmo riesce a fare una buona clusterizzazione.
...
I risultati sono identici a quelli del k-means e valgono le medesime interpretazioni.
...
```{r}
df_pca <- banknote_pca[, -1]
fviz_nbclust(df_pca, pam, method = "wss") +
  geom_vline(xintercept = 2, linetype = 2)+
  labs(subtitle = "Elbow method")
```
...
Di seguito, vengono mostrati graficamente i due cluster prodotti con l'algoritmo.

```{r}
final_pca <- eclust(df_pca, "pam", k = 2, nstart = 25, graph = FALSE)
...
Procediamo, anche nel caso del dataset con le variabili ricavate tramite mediante la PCA, a valutare la qualità del clustering k-medoids calcolando gli indici il **silhouette score**, il **Dunn index**, il **Corrected Rand Index** e il **Meila’s VI index**.
...
```{r}
pam_stats <- cluster.stats(distanze, final_pca$cluster)
pam_stats$dunn                           
```
...
- Adjusted Rand Index (ARI) che misura il grado di concordanza tra il clustering ottenuto e la classificazione reale.  
