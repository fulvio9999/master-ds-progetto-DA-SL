...
```
Il dendrogramma risultante mostra come i punti dati vengono uniti progressivamente in cluster. L'altezza dei rami indica la distanza a cui avviene la fusione: rami più bassi corrispondono a fusioni tra punti o cluster molto simili. Analizzando il dendrogramma, possiamo decidere il
...
Valutiamo ora la qualità dei risultati del nostro algoritmo di clustering mediante tre metriche: Silhouette, Dunn index e Corrected rand index.
...
diana_clustering <- eclust(df, "diana", k = 2, hc_metric = "euclidean", graph = FALSE)

fviz_dend(res.diana, cex = 0.6, show_labels = FALSE, as.ggplot = TRUE)
```
